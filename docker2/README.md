...
参见[README.md](https://github.com/copyhubduo/zotero-pdf2zh/blob/main/README.md)(#第四步-下载并安装插件)，步骤完全一致。
...
1.  额外下载 `docker-compose.dev.yaml` 和 `server` 文件夹。
    ...
    # 下载并解压 server 文件夹
    wget https://github.com/copyhubduo/zotero-pdf2zh/releases/download/v4.0.0/server.zip
    unzip server.zip
    ```
2.  使用 `-f` 参数指定配置文件启动：
`shell
